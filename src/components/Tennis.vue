<template>
  <div class="zhedie footballShow" v-if="matchesMsg != '' && !matchesMsg.playMaintenance">
    ...
    <div class="border" v-for="(matches, teamName) in matchesMsg" :key="teamName" :ref="matches.showContent">
      ...
      <div :class="matches.maxHeight" v-for="(matchesInfo,key) in matches" :key="key+'t'+matchesInfo.id">
        <!--<div class="ball_nei">-->
          <!--<span class="ball_title">-->
              <!--&nbsp;-->
              <!--<span style="margin-left: 8px;float: left" v-show="matchesInfo.showdelay!=='Y' && dataType != 1">{{forMatDate(matchesInfo.startTime)}}</span>-->
              <!--<span style="color:#ff9200;" v-show="matchesInfo.showdelay==='Y'">天气延赛</span>-->
              <!--<span v-show="matchesInfo.isLive == 1" class="data-Live">滚球</span>-->
          <!--</span>-->
          <!--<span class="ball_rq">独赢</span>-->
          <!--<span class="ball_dx">让盘</span>-->
        <!--</div>-->
        ...
        <div class="ball_cont">
          <div class="ball_dan_c">
            ...
            <!--  <router-link class='link'
                           :to="{path:'/moreTYWQ',query:{url:'/moreTYWQ',mdzz:1, type:dataType, params:{obj1: matchesInfo, obj2: reqParams}}}">-->
                <div class="dc_nei">
                    <span v-if="dateType == 1" class="banchang">
                      盘({{matchesInfo.scoreGameH}}-{{matchesInfo.scoreGameC}})局({{matchesInfo.scoreSetH}}-{{matchesInfo.scoreSetC}})
                    </span>
                  <span v-else class="banchang">{{matchesInfo.showdelay == 'Y' ? '天气延赛':forMatDate(matchesInfo.startTime)}}
                    </span>
                  <span class="add_f">+{{matchesInfo.morePlayNum ? matchesInfo.morePlayNum : '0'}} <i class="arrows"></i></span></div>
                <div class="teama"><p name="team">{{matchesInfo.hteam}}</p><span class="win">{{matchesInfo.hscore === -1 ? "A" : matchesInfo.hscore}}</span></div>
                <div class="teama"><p name="team">{{matchesInfo.gteam}}</p><span class="win">{{matchesInfo.gscore === -1 ? "A" : matchesInfo.gscore}}</span></div>
            <!--  </router-link>-->
              </div>
            </div>
            <div class="ball_dan_cnt">
              <div class="rqa">
                <p  class="team_rq" :class="{ggSelect: isMix == 1 && checkedMatches[matchesInfo.onlykey + matchesInfo.matchesDetailId + playType.rp + (matchesInfo.dStrong === 'H' ? '0' : '2')]}" to='/' @click="betDialogHandler(matchesInfo,playType.host,playType.rp,'rqz',matchesInfo.dIorRH)">
                  <span class="rq_b">
                    <i class="d-rq-l" v-if="matchesInfo.dStrong == 'H'">{{(playType.rp in gamePlayObj)? matchesInfo.dRatio:''}}</i><i></i>
                  </span>
                  <span v-if="oddsSelect() === 'oddsCss1' && matchesInfo.dIorRH" :class="matchesInfo.dIorRH <0 ?'rq_zN':'rq_z'"
                        ...
                        {{(playType.rp in gamePlayObj)? formatOdds(matchesInfo.dIorRH):'-'}}
                      </span>
              ...
              <div class="rqb">
                <p  class="team_rq" :class="{ggSelect: isMix == 1 && checkedMatches[matchesInfo.onlykey + matchesInfo.matchesDetailId + playType.rp + (matchesInfo.dStrong === 'H' ? '1' : '3')]}"  to='/' @click="betDialogHandler(matchesInfo,!playType.host,playType.rp,'rqk',matchesInfo.dIorRC)">
                  <span class="rq_b">
                    <i class="d-rq-l" v-if="matchesInfo.dStrong == 'C'">{{(playType.rp in gamePlayObj)? matchesInfo.dRatio:''}}</i><i></i>
                  </span>
                  <span v-if="oddsSelect() === 'oddsCss1' && matchesInfo.dIorRC" :class="matchesInfo.dIorRC <0 ?'rq_zN':'rq_z'"
                        ...
                        {{(playType.rp in gamePlayObj)? formatOdds(matchesInfo.dIorRC):'-'}}
                      </span>
